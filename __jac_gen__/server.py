...
@_Jac.make_walker(on_entry=[_Jac.DSFunc('return_message', _Jac.RootType)], on_exit=[])
@__jac_dataclass__(eq=False)
class multiply(_Jac.Walker):
    a: int
    b: int

    def return_message(self, _jac_here_: _Jac.RootType) -> None:
        res = self.a * self.b
        _Jac.report({'response': 'Product = ' + str(res)})

@_Jac.make_walker(on_entry=[_Jac.DSFunc('return_message', _Jac.RootType)], on_exit=[])
@__jac_dataclass__(eq=False)
class guess_number(_Jac.Walker):
    guess: int

    def return_message(self, _jac_here_: _Jac.RootType) -> None:
        if self.guess == 7:
            _Jac.report({'response': 'You win!'})
        else:
            _Jac.report({'response': 'Terrible guess. Try again.'})